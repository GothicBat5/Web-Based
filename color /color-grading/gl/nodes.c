                               ...
                               GimpLayerColorSpace composite_space)
{
  ...
  color = gegl_node_new_child (node, "operation", "gegl:color",
                               "value", background,"format", gimp_layer_mode_get_format 
                               (GIMP_LAYER_MODE_NORMAL, GIMP_LAYER_COLOR_SPACE_AUTO,
                                              composite_space,
                                     ...
                                     gdouble opacity)
{
  ...
  gegl_node_connect (mask_source, "output", opacity_node, "aux");
                             ...
                             gint offset_y)
{
  ...
  if (offset_x != 0 || offset_y != 0)
    {
      GeglNode *translate =
        gegl_node_new_child (parent, "operation", "gegl:translate",
                             "x", (gdouble) offset_x,
                             "y", (gdouble) offset_y, NULL);
...
void
gimp_gegl_node_set_color (GeglNode  *node, GeglColor *color)
{
  g_return_if_fail (GEGL_IS_NODE (node));
  g_return_if_fail (GEGL_IS_COLOR (color));

  gegl_node_set (node, "value", color, NULL);
}
