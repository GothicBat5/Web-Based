...
gimp_dash_pattern_new_from_segments (const gboolean *segments,
                                     gint n_segments, gdouble dash_length)
{
  GArray *pattern;
  gint i;
  gint count;
  ...
  {
      if (i < n_segments && segments[i] == state) count++;
                                 ...
                                 gint n_segments)
{
  ...
  {
      for (i = 0; i < n_segments; i++) segments[i] = TRUE;
      return;
  }
...
GArray *
gimp_dash_pattern_from_double_array (gint n_dashes, const gdouble *dashes)
{
...
gdouble *
gimp_dash_pattern_to_double_array (GArray *pattern, gsize *n_dashes)

{
  if (pattern != NULL && pattern->len > 0)
  {
      ...
      if (n_dashes) *n_dashes = pattern->len;
...
{
  if (pattern) g_array_free (pattern, TRUE);
}
